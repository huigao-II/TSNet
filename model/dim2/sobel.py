...
def pre_edge(im):
    ...
    sobel_kernel = np.array([[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]], dtype='float32') #0°
    sobel_kernel = sobel_kernel.reshape((1, 1, 3, 3))
    conv_op.weight.data = torch.from_numpy(sobel_kernel).cuda()
    edge_detect = torch.abs(conv_op(Variable(im)))

    conv_op1 = nn.Conv2d(1, 1, kernel_size=3, padding=1, bias=False)
    sobel_kernel1 = np.array([[1, 2, 1], [0, 0, 0], [-1, -2, -1]], dtype='float32') #90°
    ...
    conv_op2 = nn.Conv2d(1, 1, kernel_size=3, padding=1, bias=False)
    sobel_kernel2 = np.array([[0, -1, -2], [1, 0, -1], [2, 1, 0]], dtype='float32') #135°
    ...
    conv_op3 = nn.Conv2d(1, 1, kernel_size=3, padding=1, bias=False)
    sobel_kernel3 = np.array([[2, 1, 0], [1, 0, -1], [0, -1, -2]], dtype='float32') # 45°
    ...
    sobel_out = sobel_out.view(im.size())
    return sobel_out

def edge_conv2d(im):

    conv_op = nn.Conv2d(1, 3, kernel_size=3, padding=1, bias=False)
    ...
    conv_op1 = nn.Conv2d(1, 3, kernel_size=3, padding=1, bias=False)
    ...
    conv_op2 = nn.Conv2d(1, 3, kernel_size=3, padding=1, bias=False)
    ...
    conv_op3 = nn.Conv2d(1, 3, kernel_size=3, padding=1, bias=False)
