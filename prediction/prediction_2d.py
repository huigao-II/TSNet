...
def get_parser():
    ...
    parser.add_argument('--frameN', type=str, default=8, help='stack frame number')

    parser.add_argument('--load', type=parse_model_list, default='./exp/CAG/vss/best.pth',
                        help='the path of trained model checkpoint. Use \',\' as the separator if load multiple checkpoints for ensemble')
    parser.add_argument('--img_path', type=str, default='../data/label',
...
if __name__ == '__main__':
    ...
    for path in (subdir_path_list):
        ...
        for item in lab_path_list:
            ...
            trainimg_list.append(img)  # 一个序列的img
            trainlab_list.append(lab)  # 一个序列的lab
        ...
        #     img3d = torch.from_numpy(img3d)  # 从numpy转换成tensor
            ...
            save_path = os.path.join(labsave_path, img_path.split("/")[-1])
            cv2.imwrite(os.path.join(labsave_path, img_path.split("/")[-1]), pre)
