...
#model_list = ["unet_latest","Angionet_latest","SVSNet_latest","attention_unet_latest","swinunet",]
for item in model_list:
    # if item!='3f' and item!='Angionet' and item!='attention_unet' and item!='Angionet_latest':
    # if item == "TSNet" and item=="Angionet" and:

        print("calculate {} model".format(item))
        pr_path =os.path.join("result",item)
            ...
            for xulie in xulie_list:
                ...
                for frame in frame_list:
                    ...
                    if os.path.exists(primg_path):
                        ...
                        # err_list.append(err)
                        ####################计算vc
                        vc = count_connect_component(pr,gt)
                        vc_list.append(vc)
        ...
        # pre = per_class_Precision(hist)[1]
        # re = per_class_PA_Recall(hist)[1]
        # biou = np.mean(biou_list)
        # cl = np.mean(cl_list)
        # bettierr = np.mean(err_list)

        # f = open("newscore.txt","a")
        # f.write(item+" "+ str(dice)+" "+ str(Acc) + " "+str(Iou)+" "+ str(pre)+" "+ str(re)+" "+ str(biou)+" "+ str(cl)+" "+ str(bettierr)+"\n")
        # f.close()


        total_vc = np.mean(vc_list)
        f = open("vc.txt","a")
        f.write(item+" "+str(total_vc)+"\n")
        f.close
