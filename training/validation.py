...
from inference.utils import get_inference,multiget_inference
...
from .losses import DiceLoss, soft_cldice,FocalLoss
from .boundary_loss import SurfaceLoss,boundary_loss
...
def validation(net, dataloader, args):
    ...
    with torch.no_grad():
        ...
        for (images, labels) in iterator:
            ...
            inputs, labels = images.float().cuda(), labels.cuda().long()

            labels = labels.squeeze(1)
            # inputs = inputs.unsqueeze(1)
            # labels = labels.unsqueeze(1)
        #######多路编码器验证
        # for (images, labels,img_list,img_y) in iterator:
            ...
            _, label_pred = torch.max(pred_, dim=1)
            ...
            #loss = criterion(pred, labels) + criterion_dl(pred, labels.unsqueeze(1)) 
    ...
    return np.array(out_dice)


def validation_multi(net, dataloader, args):
    net.eval()
    ...
    inference = multiget_inference(args)
    ...
    with torch.no_grad():
        iterator = tqdm(dataloader)
        ##### 单路编码器验证
        for (img,labels,img_y) in iterator:

            inputs, labels = img.float().cuda(), labels.cuda().to(torch.int8)
            ...
            pred = inference(net, img,img_y, args)
