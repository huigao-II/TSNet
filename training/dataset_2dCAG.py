...
class CAGDataset(Dataset):
        ...
        for path in img_path_list:
            ...
            for k in range(t):
                ...
                for n in range(self.frame):
                    stack_imgpath_list.append(self.img_path_list[n+k*5+w])
                    stack_labpath_list.append(self.img_path_list[n+k*5+w].replace("image","label"))
                self.imgpath_list.append(stack_imgpath_list)
                self.labpath_list.append(stack_labpath_list)

                    
        self.augimgpath_list.extend(self.imgpath_list)
        self.auglabpath_list.extend(self.labpath_list)
        # assert len(self.auglabpath_list) == len(self.augimgpath_list)
    ...
    def __getitem__(self, idx):
        ...
        if self.args.dimension == '2d':
            ...
            # lab_path = self.auglabpath_list[idx]
            # lab = cv2.imread(lab_path,0)
            # lab = lab//255
            # lab = torch.from_numpy(lab).long()
            ...
            for s in img_path:
                ...
                imgstack_list.append(img_slice)

                lab_slice = cv2.imread(s.replace("image","label"),0)
                lab_slice = lab_slice//255
                lab_slice = torch.from_numpy(lab_slice).long()
                labstack_list.append(lab_slice) 

            img =torch.stack(imgstack_list)
            lab =torch.stack(labstack_list)


            # labstack_list = []
            # for s in lab_path:
            #     lab_slice = cv2.imread(s,0)
            #     lab_slice = lab_slice//255
            #     lab_slice = torch.from_numpy(lab_slice).long()
            #     labstack_list.append(lab_slice) 
            # lab =torch.stack(labstack_list)
